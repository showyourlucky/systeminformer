...
INT_PTR CALLBACK EtpNpuNodesDlgProc(
    _In_ HWND hwndDlg,
    _In_ UINT uMsg,
    _In_ WPARAM wParam,
    _In_ LPARAM lParam
    ...
    )
{
    ...
    EtNpuNodesWindowHandle = PhCreateDialog(
        PluginInstance->DllBase,
        MAKEINTRESOURCE(IDD_NPUNODES),
        NULL,
        EtpNpuNodesDlgProc,
        Parameter
        );

    PhSetEvent(&EtNpuNodesInitializedEvent);
    ...
    while (result = GetMessage(&message, NULL, 0, 0))
    {
        ...
        if (!IsDialogMessage(EtNpuNodesWindowHandle, &message))
        {
    ...
    PhResetEvent(&EtNpuNodesInitializedEvent);

    if (EtNpuNodesThreadHandle)
        NtClose(EtNpuNodesThreadHandle);

    EtNpuNodesThreadHandle = NULL;
    EtNpuNodesWindowHandle = NULL;
    ...
    )
{
    if (!EtNpuNodesThreadHandle)
    {
        ...
        PhWaitForEvent(&EtNpuNodesInitializedEvent, NULL);
    }

    PostMessage(EtNpuNodesWindowHandle, WM_PH_SHOW_DIALOG, 0, 0);
}
    ...
    )
{
    ...
    case WM_INITDIALOG:
        {
            ...
            GraphHandle = PhAllocate(sizeof(HWND) * EtNpuTotalNodeCount);
            GraphState = PhAllocate(sizeof(PH_GRAPH_STATE) * EtNpuTotalNodeCount);
            ...
            numberOfRows = (ULONG)sqrt(EtNpuTotalNodeCount);
            numberOfColumns = (EtNpuTotalNodeCount + numberOfRows - 1) / numberOfRows;
    ...
    case WM_DESTROY:
        {
            PhSaveWindowPlacementToSetting(SETTING_NAME_NPU_NODES_WINDOW_POSITION, SETTING_NAME_NPU_NODES_WINDOW_SIZE, hwndDlg);
    ...
    case WM_SIZE:
        {
            HDWP deferHandle;
            RECT clientRect;
            ULONG numberOfRows = (ULONG)sqrt(EtNpuTotalNodeCount);
            ULONG numberOfColumns = (EtNpuTotalNodeCount + numberOfRows - 1) / numberOfRows;
